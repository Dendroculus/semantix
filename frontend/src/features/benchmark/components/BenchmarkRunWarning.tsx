  ...
  return (
    ...
    >
      ...
      <p className="mt-3 max-w-3xl text-sm/6 text-(--text-soft)">
        ...
        {formatCount(queryCount)} external generation calls; dataset labels
        estimate about {formatCount(estimatedProviderCalls)}. Actual calls can
        differ when cache decisions are false positives or false negatives.
        Imported prompts may leave this system through the configured
        embedding or generation provider. Provider charges may apply.
      </p>
      ...
      <p className="font-data mt-2 text-[10px]/5 text-(--text-faint)">
        ...
        {formatUsd(controller.form.costPer1kTokensUsd, 4)} per 1K estimated
        tokens. These inputs are estimates, not billing records.
      </p>
