...
export function QueryLog({ traces, threshold }: QueryLogProps): JSX.Element {
  return (
    <section className="mt-16 border-t border-[var(--hairline)] pt-8">
      ...
      {traces.length === 0 ? (
        <p className="font-data border-y border-[var(--hairline)] py-5 text-[11px] leading-5 text-[var(--text-muted)]">
          NO QUERY TRACES YET / THE FIRST PROBE WILL LAND AT ITS NEAREST CACHE NEIGHBOR
        </p>
      ) : (
        <div>
          <div className="ui-label hidden grid-cols-[90px_72px_minmax(180px,1fr)_72px_90px] gap-4 border-b border-[var(--hairline)] pb-3 text-[var(--text-faint)] min-[760px]:grid">
            <span>Time</span>
            <span>Score</span>
            <span>Query</span>
            <span>Verdict</span>
