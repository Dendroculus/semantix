...
describe('BenchmarkAnalysis', () => {
  ...
  it('supports false-positive, false-negative, search, and empty quick paths', () => {
    const { rerender } = render(
      <BenchmarkAnalysis result={benchmarkAnalysisResult} />,
    );

    fireEvent.click(
      screen.getByRole('button', { name: 'False positives' }),
    );
    expect(within(measuredRows()[1] as HTMLElement).getByText('shared-miss')).toBeTruthy();

    fireEvent.click(
      screen.getByRole('button', { name: 'False negatives' }),
    );
    expect(within(measuredRows()[1] as HTMLElement).getByText('shared-hit')).toBeTruthy();
    ...
    expect(measuredRows()).toHaveLength(2);
    expect(within(measuredRows()[1] as HTMLElement).getByText('false negative')).toBeTruthy();

    rerender(<BenchmarkAnalysis result={benchmarkResult} />);
    fireEvent.click(
      screen.getByRole('button', { name: 'False positive: 0 cases' }),
    );
    expect(
      screen.getByText(
