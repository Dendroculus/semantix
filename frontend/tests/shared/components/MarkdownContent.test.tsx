...
describe("MarkdownContent URL safety", () => {
  ...
  it.each([
    ...
    render(
      <MarkdownContent markdown={`[Unsafe link](${url})`} />,
    );

    const renderedAnchor =
      screen.getByText("Unsafe link").closest("a");
  ...
  it("removes data URLs from Markdown images", () => {
    render(
      <MarkdownContent
        markdown="![Unsafe image](data:image/svg+xml;base64,PHN2Zz4=)"
      />,
    );

    expect(
      screen
        .getByRole("img", { name: "Unsafe image" })
        .getAttribute("src"),
    ).toBe("");
  });
