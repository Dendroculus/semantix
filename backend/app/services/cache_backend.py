...
class InMemoryCacheBackend:
    ...
    def _purge(self) -> None:
        now = time.monotonic()
        for key in [key for key, item in self._items.items() if item.expires_at is not None and item.expires_at <= now]:
    ...
    async def find_nearest(self, embedding: Sequence[float]) -> CacheCandidate | None:
        ...
        async with self._lock:
            ...
            matrix: NDArray[np.float64] = np.asarray([item.entry.embedding for item in items], dtype=np.float64)
            norms = np.linalg.norm(matrix, axis=1)
            query_norm = float(np.linalg.norm(query))
            if query_norm <= np.finfo(np.float64).eps or np.any(norms <= np.finfo(np.float64).eps):
                raise CacheStorageError("Zero magnitude embedding")
            scores = (matrix @ query) / (norms * query_norm)
            index = int(np.argmax(scores))
            return CacheCandidate(entry=items[index].entry.model_copy(deep=True), similarity_score=max(-1.0,min(1.0,float(scores[index]))))
    async def put(self, entry: CacheEntry) -> None:
        async with self._lock:
            self._purge()
            expiry = None if self._ttl_seconds is None else time.monotonic() + self._ttl_seconds
            self._items[entry.cache_key] = _StoredItem(entry.model_copy(deep=True), expiry)
    ...
    async def stats(self) -> CacheStatsResponse:
        async with self._lock:
            ...
            return CacheStatsResponse(size=len(self._items), hits=self._hits, misses=self._misses, hit_rate=0.0 if total == 0 else self._hits / total)
