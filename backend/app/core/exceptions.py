...
class EmbeddingError(AppError):
    status_code, error_code, public_detail = 502, "embedding_error", "The embedding service returned an invalid result."
class CacheStorageError(AppError):
    status_code, error_code, public_detail = 500, "cache_error", "The cache could not process the request."
class ProviderRetryableError(AppError):
    status_code, error_code, public_detail = 503, "service_unavailable", "The AI service is temporarily unavailable."
class ProviderAuthenticationError(AppError):
    status_code, error_code, public_detail = 503, "service_configuration_error", "The AI service is temporarily unavailable."
class ProviderRequestError(AppError):
    status_code, error_code, public_detail = 502, "upstream_error", "The AI service could not process the request."
class InvalidProviderResponseError(AppError):
    status_code, error_code, public_detail = 502, "invalid_upstream_response", "The AI service returned an invalid response."
...
async def validation_error_handler(request: Request, exc: Exception) -> JSONResponse:
    ...
    return _response(422, "validation_error", "Invalid field: " + ", ".join(sorted(locations)))
...
async def http_error_handler(request: Request, exc: Exception) -> JSONResponse:
    ...
    return _response(error.status_code, "http_error", error.detail if isinstance(error.detail, str) else None)
