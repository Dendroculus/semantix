...
async def cache_stats(request:Request,cache:Annotated[SemanticCache,Depends(get_semantic_cache)])->CacheStatsResponse:
    return await cache.stats()
@router.delete("",response_model=ClearCacheResponse)
@limiter.limit(lambda:get_settings().rate_limit)  # type: ignore[untyped-decorator]
async def clear_cache(request:Request,cache:Annotated[SemanticCache,Depends(get_semantic_cache)])->ClearCacheResponse:
    await cache.clear()
    return ClearCacheResponse(cleared=True)
@router.get(
    "/threshold",
    response_model=CacheThresholdResponse,
)
...
async def update_threshold(
    ...
    return CacheThresholdResponse(
        threshold=cache.update_similarity_threshold(
            payload.threshold
        )
    )