...
def vector(index:int)->list[float]:
    result=[0.0]*EMBEDDING_DIMENSIONS;result[index]=1.0;return result
class Embeddings:
    async def embed(self,text:str)->Sequence[float]:return vector(0) if text in {"one","similar"} else vector(1)
@pytest.mark.asyncio
async def test_semantic_hit()->None:
    cache=SemanticCache(Embeddings(),InMemoryCacheBackend(10,60),0.92)
    miss=await cache.lookup("one");await cache.store("one","answer",miss.embedding)
...
async def test_clear_resets_stats()->None:
    cache=SemanticCache(Embeddings(),InMemoryCacheBackend(10,60),0.92)
    await cache.lookup("one");await cache.clear()
